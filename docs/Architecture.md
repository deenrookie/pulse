...
> 本文主体保留早期架构说明。当前生产 UI 默认端口为 8787，开发代理后端为 8000、Vite 为 5175。HTTP/2、WebSocket 帧、双向拦截及插件主动能力已实现，最新功能与限制见 README 和 Plugins.md。本轮新增：API 磁盘持久化临时分享（独立公开快照路由、原管理 gate 保持）、内嵌插件 Skill 与 ZIP 下载；Store 发布独立元数据快照以防生产者修改导致读写竞争，正文按不可变字节共享。具体流程和边界见 [优化方案](Optimization-2026-09.md)。
